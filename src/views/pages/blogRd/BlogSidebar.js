...
const BlogSidebar = () => {
  ...
  useEffect(() => {
    axios.get('/blogRd/list/data/sidebar').then(res => setData(res.data))
  }, [])
  ...
  const renderRecentPosts = () => {
    return data.recentPosts.map((post, index) => {
      return (
        ...
        >
          <Link className='mr-2' to={`/pages/blogRd/detail/${post.id}`}>
            <img className='rounded' src={post.img} alt={post.title} width='100' height='70' />
          </Link>
          <Media body>
            <h6 className='blog-recent-post-title'>
              <Link className='text-body-heading' to={`/pages/blogRd/detail/${post.id}`}>
                {post.title}
              </Link>
            </h6>
